...
public class CustomString {
	...
	public String reverse(String arg) {
		...
		for(int i=0;i<temp.length();i++){
			if(arg.contains(temp.charAt(i)+"")&& Character.isAlphabetic(temp.charAt(i))){
			if(Character.isUpperCase(temp.charAt(i)))
			output+=((temp.charAt(i)+"").toLowerCase());
			else
	...
	public String filterLetters(char n, boolean more) {
		...
		if(more){
		for(int i=0;i<temp.length();i++)
			if(!Character.isAlphabetic(temp.charAt(i))|| n>=temp.charAt(i))
			output+=(temp.charAt(i));
		}
		else{
		for(int i=0;i<temp.length();i++)
